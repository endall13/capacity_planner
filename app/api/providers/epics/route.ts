...
export async function GET(req: NextRequest) {
  ...
  const providerProjectId = req.nextUrl.searchParams.get("providerProjectId");
  if (!integrationId || !providerProjectId) {
    return Errors.BAD_REQUEST("integrationId and providerProjectId are required");
  }
  ...
  try {
    const epics = await getProvider(integration).getEpics(providerProjectId);
